...
@ToolParserManager.register_module("ax")
class AxToolCallParser(ToolParser):
    """
    Tool call parser for the ax model format: <tool_call>{"name": "function_name", "arguments": {...}}</tool_call>
    """

    def __init__(self, tokenizer: AnyTokenizer):
        super().__init__(tokenizer)
        
        # Regex pattern to match <tool_call>{...}</tool_call>
    ...
    def extract_tool_calls_streaming(
        ...
        try:
            ...
            if current_num_calls > prev_num_calls:
                ...
                try:
                    tool_call_data = json.loads(latest_match)
                    function_name = tool_call_data.get("name", "")
                    arguments = tool_call_data.get("arguments", {})
